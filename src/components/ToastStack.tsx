...
export function ToastStack({ toasts }: ToastStackProps) {
  return (
    <div className="fixed right-5 top-5 z-50 grid w-[320px] max-w-[calc(100vw-32px)] gap-2">
      {toasts.map((toast) => (
        <div className="toast-item rounded-xl border border-[var(--border-2)] bg-[var(--panel-solid)] p-4 shadow-2xl" key={toast.id}>
          <div className="mb-2 flex items-center justify-between gap-3">
            <strong className="text-[13px]">{toast.title}</strong>
            <span
              aria-hidden="true"
              className="toast-tone-dot"
              style={{ background: `var(--${toast.tone ?? "green"})`, color: `var(--${toast.tone ?? "green"})` }}
            />
          </div>
